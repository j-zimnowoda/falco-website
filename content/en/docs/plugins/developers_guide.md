...
Extractor plugins need to import `sdk/plugins/extractor` only. Instead, source plugins can either choose to just import `sdk/plugins/source` or to additionally import `sdk/plugins/extractor` too, to enable the optional field extraction features.
...
On top of the mandatory plugin interface requirements, source and extraction plugins can also implement some additional interface method to enable optional functionalities.
...
Source plugins can optionally define the two extraction-related `Fields()` and `Extract()` methods documented above. This is a requirement to enable the optional extraction features of source plugins.
...
```
The newly created plugin type need to be registered to the SDK in a Go `init` function. The `source.Register()` and `extractor.Register()` functions register a source plugin and an extractor plugin respectively. By calling both of them, the SDK will register a source plugin with the optional extraction features enabled. The order in which the two `Register()` functions are called is not relevant.
...
The `sdk/plugins/source` and `sdk/plugins/extractor` packages are required to register the functionalities of source and extractor plugins. `dummy` requires on both of them, as it is an example of source plugin with the optional extraction features enabled.
...
The `Fields()` method returns a slice of `sdk.FieldEntry` representing all the supported fields. `Fields()` is mandatory for extractor plugins, and for source plugins with the optional extraction features.
...
The `Extractor` method extracts any of the supported fields from a given event, and is mandatory for extractor plugins, and for source plugins with the optional extraction features. The `req` parameter allows accessing all the info regarding the field request, such as the field id or name, and the optional user-passed argument. The `evt` parameter is an interface that halps reading the event info and data.
